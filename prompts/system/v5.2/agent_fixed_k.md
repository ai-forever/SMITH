...
Search:
- Each search returns the configured top {agent_k} passages; you may use at most {max_search_calls} searches.
- Call `search_index` with the query only; the configured top-k is fixed.
