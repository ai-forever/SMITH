...
After retrieval is complete, finish by calling the `{final_answer_tool}` tool exactly once.
Do not emit free-form JSON in the final assistant message.
Only include supporting corpus ids that were returned by the tool in the current conversation.
